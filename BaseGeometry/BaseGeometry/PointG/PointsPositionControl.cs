    ...
    public class PointsPositionControl
    {
        ...
        public bool PointsIsPoints(Point2D[] Points)//Реализовать в 2D и 3D классах + Реализовать совпадение точек в 2D и 3D классах и плоскостей
        {
            ...
            for (i = 0; i <= Points.GetUpperBound(0) - 1; i++)
            {
                ...
                if (this.PointsIsPoints(Points[i], Points[i + 1]) == true) { BoolVar = true; }
        ...
        public bool PointsIsPoints(Point2D[] Points, double SolveError)//Реализовать в 2D и 3D классах + Реализовать совпадение точек в 2D и 3D классах и плоскостей
        {
            ...
            {
                if (i == Points.GetUpperBound(0)) { break; }
                if (this.PointsIsPoints(Points[i], Points[i + 1], SolveError) == true) { BoolVar = true; }
                else { BoolVar = false; break; }
            }
            return BoolVar;
        }
        ...
        /// <remarks></remarks>
        public bool PointsIsPoints(Point3D Point1, Point3D Point2) { if (this.PointsIsPoints(Point1, Point2) == true & Math.Abs(Point1.Z - Point2.Z) == 0) { return true; } else { return false; } }

        /// <summary>Возвращает значение "TRUE", если совпадают заданные 3D точки</summary>
        /// <remarks>Параметр "SolveError" устанавливает погрешность расчета</remarks>
        public bool PointsIsPoints(Point3D Point1, Point3D Point2, double SolveError) { if (this.PointsIsPoints(Point1, Point2, SolveError) == true & Math.Abs(Point1.Z - Point2.Z) <= SolveError) { return true; } else { return false; } }
        ...
        public bool PointsIsPoints(GeomObjects.Points.PointOfPlan1X0Y PointProjection_1, GeomObjects.Points.PointOfPlan1X0Y PointProjection_2) { return this.PointsIsPoints(PointOfPlan1X0YVar.CnvPoint2D(PointProjection_1), PointOfPlan1X0YVar.CnvPoint2D(PointProjection_2)); }
        ...
        /// <returns>Возвращает "True" при совпадении положения двух заданных проекций точек. В противном случае возвращает "False"</returns>
        /// <remarks>Точность расчетов не регламентируется</remarks>
        public bool PointsIsPoints(GeomObjects.Points.PointOfPlan2X0Z PointProjection_1, GeomObjects.Points.PointOfPlan2X0Z PointProjection_2) { return this.PointsIsPoints(PointOfPlan2_X0Z_Var.CnvPoint2D(PointProjection_1), PointOfPlan2_X0Z_Var.CnvPoint2D(PointProjection_2)); }
        ...
        /// <returns>Возвращает "True" при совпадении положения двух заданных проекций точек. В противном случае возвращает "False"</returns>
        /// <remarks>Точность расчетов не регламентируется</remarks>
        public bool PointsIsPoints(GeomObjects.Points.PointOfPlan3Y0Z PointProjection_1, GeomObjects.Points.PointOfPlan3Y0Z PointProjection_2) { return this.PointsIsPoints(PointOfPlan3_Y0Z_Var.CnvPoint2D(PointProjection_1), PointOfPlan3_Y0Z_Var.CnvPoint2D(PointProjection_2)); }
        ...
        public Enum CoordinateIsValue(GeomObjects.Points.Point3D Point3D, double Value)//Контроль значений координат точки заданному значению //Возвращает метку, соответсвующую координатам, для которых соответсвие выполняется
        {
        ...
        public bool PointMinus(GeomObjects.Points.Point3D Point3D)//Контроль отрицательных координат
        {
            Enum EnumPointError = PointError.Minus;//Исходное значение нумератора
            return this.PointMinus(Point3D, EnumPointError);
        }
        ...
        public bool PointMinus(GeomObjects.Points.Point3D Point3D, Enum EnumPointError)//Контроль отрицательных координат
        {
            bool Ptbool = false; string CommentPt_Var = null; Ptbool = this.PointMinus(Point3D, CommentPt_Var, EnumPointError);
        ...
        public bool PointMinus(GeomObjects.Points.Point3D Point3D, bool MessagerBoxShow)//Контроль отрицательных координат
        {
            bool MinusBool = false; Enum EnumPointError = PointError.Minus;//Исходное значение нумератора
            if (MessagerBoxShow == true) { MinusBool = this.PointMinus(Point3D, EnumPointError); }
            else { MinusBool = this.PointMinus(Point3D, null, EnumPointError); }
            return MinusBool;
        }
        ...
        /// <param name="EnumPointError"> Значение перечисления, указывающего индексы координат, имеющих отрицательные значения</param>
        public bool PointMinus(GeomObjects.Points.Point3D Point3D, string CommentMinus, Enum EnumPointError)//Контроль отрицательных координат
        {
