...
namespace GeomObjects.Points
{
    ...
    public class Point2D
    {
        ...
        public Point2D() { this.X = 0; this.Y = 0; this.SolveError = 0.001; }//Конструктор, устанавливающий исходные значения координат 2D точки

        /// <summary>Инициализирует новый экземпляр 2D точки с указанными координатами</summary>
        /// <remarks></remarks>
        public Point2D(double X, double Y) { this.X = X; this.Y = Y; this.SolveError = 0.001; }

        /// <summary>Инициализирует новый экземпляр 2D точки</summary>
        /// <remarks></remarks>
        public Point2D(Point2D pt) { this.X = pt.X; this.Y = pt.Y; this.SolveError = 0.001; }//Конструктор, устанавливающий пользовательские значения координат 2D точки
        ...
        public void PointMove(double dx, double dy) { this.X += dx; this.Y += dy; }//Конструктор перемещения на указанные величины по осям //MyClass.Ptcls.X += dx : MyClass.Ptcls.Y += dy
        ...
        {  //Возвращает метку, соответсвующую координатам, для которых значения координат соответсвуют значению "Nothing"
            ...
            if (Xbool & Ybool) { ProectionError = Points.PointsPositionControl.CoordinateValue.XY; }
            else if (Xbool & Ybool == false) { ProectionError = Points.PointsPositionControl.CoordinateValue.X; }
            else if (Xbool == false & Ybool) { ProectionError = Points.PointsPositionControl.CoordinateValue.Y; }//Значения координат заданы
            else
            {
                ProectionError = Points.PointsPositionControl.CoordinateValue.None;
                Points.Point2D Point2DByTextVar = new Points.Point2D(Convert.ToDouble(XText), Convert.ToDouble(YText)); return Point2DByTextVar;
            }//Точка для вывода
            return null;
        }
    }
