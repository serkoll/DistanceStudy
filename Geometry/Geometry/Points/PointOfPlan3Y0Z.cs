...
namespace GeometryObjects
{
    ...
    public class PointOfPlan3Y0Z
    {
        ...
        private Point2D SolveErrorPtcls = new Point2D();//Точность расчетов
        ...
        public PointOfPlan3Y0Z() { Y = 0; Z = 0; }//Конструктор, устанавливающий исходные значения координат двумерной проекции точки
        ...
        /// <remarks></remarks>
        public PointOfPlan3Y0Z(PointOfPlan3Y0Z pt) { Y = pt.Y; Z = pt.Z; }//Конструктор, устанавливающий пользовательские значения координат 2D точки
        ...
        /// </summary>
        public PointDraw Point3D_Draw { get { return PointDraw_Var; } set { PointDraw_Var = value; } }
        ...
        {
            get { return SolveErrorPtcls.SolveError; }
            set { SolveErrorPtcls.SolveError = value; }
        ...
        public void PointMove(double dy, double dz) { Y += dy; Z += dz; }//Конструктор перемещения на указанные величины по осям
        ...
        public GeometryObjects.PointOfPlan3Y0Z PointByText(string Y_Text, string Z_Text, Enum ProectionError)
        {//Контроль не заданных значений координат горизонтальной проекции точки //Возвращает метку, соответсвующую координатам, для которых значения координат соответсвуют значению "Nothing"
            ...
            if (Ybool & Zbool) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.YZ; }
            else if (Ybool & Zbool == false) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.Y; }
            else if (Ybool == false & Zbool) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.Z; }
            else { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.None; GeometryObjects.PointOfPlan3Y0Z Point2DByTextVar = new GeometryObjects.PointOfPlan3Y0Z(Convert.ToDouble(Y_Text), Convert.ToDouble(Z_Text)); return Point2DByTextVar; } //Значения координат заданы //Точка для вывода
            return null;
        }
        ...
        public Point2D CnvPoint2D(PointOfPlan3Y0Z PointProjection) { Point2D PointCalc = new Point2D(PointProjection.Y, PointProjection.Z); return PointCalc; }//Конструктор перемещения на указанные величины по осям
    }
}
