...
namespace GeometryObjects
{
    ...
    public class PointOfPlan1X0Y
    {
        ...
        public PointOfPlan1X0Y() { X = 0; Y = 0; }//Конструктор, устанавливающий исходные значения координат 2D точки
        ...
        /// <remarks></remarks>
        public PointOfPlan1X0Y(PointOfPlan1X0Y pt) { X = pt.X; Y = pt.Y; }//Конструктор, устанавливающий пользовательские значения координат 2D точки
        ...
        {
            get { return Point2D_Var.SolveError; }
            set { Point2D_Var.SolveError = value; }
        ...
        public GeometryObjects.PointOfPlan1X0Y PointByText(string X_Text, string Y_Text, Enum ProectionError)
        {//Контроль не заданных значений координат горизонтальной проекции точки//Возвращает метку, соответсвующую координатам, для которых значения координат соответсвуют значению "Nothing"
            ...
            if (Xbool & Ybool) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.XY; }
            else if (Xbool & Ybool == false) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.X; }
            else if (Xbool == false & Ybool) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.Y; }
            else { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.None; GeometryObjects.PointOfPlan1X0Y Point2DByTextVar = new GeometryObjects.PointOfPlan1X0Y(Convert.ToDouble(X_Text), Convert.ToDouble(Y_Text)); return Point2DByTextVar; }//Точка для вывода//Значения координат заданы
            return null;
        }
        ...
        public Point2D CnvPoint2D(PointOfPlan1X0Y PointProjection) { Point2D PointCalc = new Point2D(PointProjection.X, PointProjection.Y); return PointCalc; }//Конструктор перемещения на указанные величины по осям
