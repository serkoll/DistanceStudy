...
namespace GeometryObjects
{
    ...
    public class PointOfPlan2X0Z
    {
        ...
        public PointOfPlan2X0Z() { X = 0; Z = 0; }//Конструктор, устанавливающий исходные значения координат 2D точки
        ...
        /// <remarks></remarks>
        public PointOfPlan2X0Z(PointOfPlan2X0Z pt) { X = pt.X; Z = pt.Z; }//Конструктор, устанавливающий пользовательские значения координат 2D точки
        ...
        {//Контроль не заданных значений координат горизонтальной проекции точки//Возвращает метку, соответсвующую координатам, для которых значения координат соответсвуют значению "Nothing"
            ...
            if (Xbool & Zbool) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.XZ; }
            else if (Xbool & Zbool == false) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.X; }
            else if (Xbool == false & Zbool) { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.Z; }
            else { ProectionError = GeometryObjects.PointsPositionControl.CoordinateValue.None; PointOfPlan2X0Z Point2DByTextVar = new PointOfPlan2X0Z(Convert.ToDouble(X_Text), Convert.ToDouble(Z_Text)); return Point2DByTextVar; }//Значения координат заданы//Точка для вывода
            return null;
        }
        ...
        public Point2D CnvPoint2D(PointOfPlan2X0Z PointProjection) { Point2D PointCalc = new Point2D(PointProjection.X, PointProjection.Z); return PointCalc; }//Конструктор перемещения на указанные величины по осям
    }
}
