...
namespace GraphicsModule.Rules.Create.Planes
{
    public class CreatePlaneOfPlane2X0Z : ICreate, ICreatePlanes
    {
        ...
        public void AddToStorageAndDraw(Point pt, Point frameCenter, Blueprint blueprint, DrawSettings settings, Storage storage)
        {
            switch (_creationType)
            {
                case PlaneCreateType.ThreePoints:
                    {
                        CreateByThreePoints(pt, frameCenter, blueprint, settings, storage);
                        break;
                    }
                ...
                case PlaneCreateType.SegmentAndPoint:
                    {
                        CreateBySegmentAndPoint(pt, frameCenter, blueprint, settings, storage);
                        break;
                    }
        ...
        private void CreateByThreePoints(Point pt, Point frameCenter, Blueprint can, DrawSettings setting, Storage strg)
        {
        ...
        private void CreateBySegmentAndPoint(Point pt, Point frameCenter, Blueprint can, DrawSettings setting, Storage strg)
        {
            if (_planeObjects.Count == 0)
            {
        ...
        {
            return ln1.IsCrossed(ln2, frameCenter) ? new PlaneOfPlane2X0Z(ln1, ln2) : null;
        }
        ...
        {
            return sg1.IsCrossed(sg2, frameCenter) ? new PlaneOfPlane2X0Z(sg1, sg2) : null;
        }
