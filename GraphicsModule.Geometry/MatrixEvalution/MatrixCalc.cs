...
namespace GraphicsModule.Geometry.MatrixEvalution
{
    ...
    public class MatrixEvalution
    {
        ...
        CalcMoveZeroCollsRows:
            ...
            MrxTransInt = (double[,])MrxMoveCollsZero(MrxTrans);
            MrxTrans = (double[,])MrxTransInt.Clone();
            MrxTransInt = (double[,])MrxMoveRowsZero(MrxTrans);
            MrxTrans = (double[,])MrxTransInt.Clone();
        CalcZero_ij:
            ...
            {
                MrxTransInt = (double[,])MrxMoveUnit(MrxTrans, i_0, j_0);
                MrxTrans = (double[,])MrxTransInt.Clone();
            }
        RangStep1:
            ...
            while (i <= MrxTrans.GetUpperBound(0))
            {
                if (MrxRowZero(MrxTrans, i) == true | i == MrxTrans.GetUpperBound(0))
                {
                    MrxUpperBound0 = i;
                    break; // TODO: might not be correct. Was : Exit Do
                }
            ...
            while (j <= MrxTrans.GetUpperBound(1))
            {
                if (MrxCollZero(MrxTrans, j) == true | j == MrxTrans.GetUpperBound(1))
                {
                    MrxUpperBound1 = j;
                    break; // TODO: might not be correct. Was : Exit Do
                }
                    ...
                    while (ich1 <= MrxUpperBound0)
                    {
                        if ((double)MrxTrans.GetValue(ich, jch) == 0)
                        {
                            if ((double)MrxTrans.GetValue(ich, jch) / (double)MrxTrans.GetValue(ich1, jch) == Math.Round((double)MrxTrans.GetValue(ich, jch) / (double)MrxTrans.GetValue(ich1, jch)))
                            {
                                ...
                                //    GoTo StepIch
                                //End If
                                MrxTransInt = (double[,])MrxReplaceRows(MrxTrans, i_0, ich1);
                                //Перестановка строк 
                                MrxTransInt = (double[,])MrxReplaceRows(MrxTrans, i_0 + 1, ich);
                                //Перестановка строк 
                                MrxTransInt = (double[,])MrxReplaceColls(MrxTransInt, j_0, jch);
                        ...
                        else
                        {
                            ...
                            {
                                MrxTransInt = (double[,])MrxReplaceRows(MrxTrans, i_0, ich);
                                //Перестановка строк 
                                MrxTransInt = (double[,])MrxReplaceRows(MrxTrans, i_0 + 1, ich1);
                                //Перестановка строк 
                                MrxTransInt = (double[,])MrxReplaceColls(MrxTransInt, j_0, jch);
                                //Перестановка столбцов 
                                MrxTrans = (double[,])MrxTransInt.Clone();
        ...
        RangStepIII:
            ...
            {
                if (MrxRowZero(MrxTrans, i3) == false)
                {
                    Var = false;
                    break; // TODO: might not be correct. Was : Exit Do
                }
        ...
        Calc_Step3:
            ...
            MrxTransInt = (double[,])MrxMoveCollsZero(MrxTransInt);
            MrxTrans = (double[,])MrxTransInt.Clone();
        Calc_Step3a:
        ...
        Calc_Step4б:
            ...
            while (j <= MrxTrans.GetUpperBound(1))
            {
                ...
                if (MrxCollZero(MrxTrans, j) == true)
                {
                    goto RowZero_i;
                }
                else
                {
                    ...
                    {
                        MrxTransInt = (double[,])MrxReplaceColls(MrxTrans, j, NumColl);
                        //Замена элементов по строке
                        MrxTransInt = (double[,])MrxReplaceRows(MrxTransInt, i, NumRow);
        ...
        Calc_Step2:
            ...
            while (i <= MrxTrans.GetUpperBound(0))
            {
                VarRowZero = MrxRowZero(MrxTrans, i);
        ...
        Calc_Step3:
            ...
            {
                VarRowZero = MrxRowZero(MrxTrans, i1);
                //Строка содержит не нулевые элементы 
                if (VarRowZero == false)
                {
                    MrxTransInt = (double[,])MrxReplaceRows(MrxTrans, i1, i);
                    //Перестановка строк
                    goto RowI;
                }
                i1 += 1;
        ...
        Calc_Step2:
            ...
            while (j <= MrxTrans.GetUpperBound(1))
            {
                VarСolZero = MrxCollZero(MrxTrans, j);
        ...
        Calc_Step3:
            ...
            {
                VarСolZero = MrxCollZero(MrxTrans, j1);
                //Столбец содержит не нулевые элементы 
                if (VarСolZero == false)
                {
                    MrxTransInt = (double[,])MrxReplaceColls(MrxTrans, j1, j);
                    //Перестановка столбцов
                    goto Rowj;
                }
                j1 += 1;
        ...
        public object MrxReplaceRows(double[,] Mrxix, int NumRow1, int NumRow2)
        {
            ...
            if (MrxRow1IsRow2(Mrxix, NumRow1, NumRow2) == true)
            {
                return MrxTrans;
            }
            else
        ...
        public object MrxReplaceColls(double[,] Mrxix, int NumColl_1, int NumColl_2)
        {
            ...
            MrxTrans = (double[,])Mrxix.Clone();
            //Проверка равентсва заданных строк
            if (MrxColl1IsColl2(Mrxix, NumColl_1, NumColl_2) == true)
            {
                return MrxTrans;
            }
            else
